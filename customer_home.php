...
<body>
   ...
   <h1 id="customer">
                Bienvenue, <?php echo $row0["first_name"] ?>&nbsp<?php echo $row0["last_name"] ?>&nbsp!
        
        <br>AC/No: <?php echo $row0["account_no"]; ?>
            </h1>
            <p id="customer">
              
  &#9656 Solde (INR): <?php echo number_format($row1["balance"]); ?>/-
               
 <br>&#9656 Vous Avez <?php echo $row2["COUNT(*)"]; ?> beneficiaries.
          
      <br>&#9656 Derniere Transaction (<?php echo $type; ?>) of&nbspRs.&nbsp<?php
                echo number_format($transaction); ?>
<br>
                en <?php echo $sanitized_time; ?>, etait: "<?php echo $row1["remarks"]; ?>".
            </p>
        </div>
    </div>
